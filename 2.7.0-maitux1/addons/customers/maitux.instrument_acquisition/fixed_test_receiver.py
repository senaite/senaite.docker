...
def receive():
    ...
    try:
        ...
        print("  Raw body length: %d" % len(raw_body))
        print("  Raw body: %r" % raw_body)
        
        # 尝试解析 JSON - 使用 raw_body 直接解析
        if raw_body:
            try:
...
def main():
    ...
    parser.add_argument('--port', type=int, default=5003, help='Port number (default: 5003)')
