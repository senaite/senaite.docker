...
siteinstall 是给本 addon 三处 for="*" 的进程级注册（IGuardAdapter、
IBeforeTransitionEvent 订阅器、IActionSucceededEvent 订阅器）做站点级
收口用的：未安装本 addon 的站点必须判成 False，让它们全部不参与。
"""
...
_spec = importlib.util.spec_from_file_location("es_siteinstall", MODULE_PATH)
siteinstall = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(siteinstall)
...
class TestProfileVersionPredicate(unittest.TestCase):
    ...
    def test_version_tuple_is_installed(self):
        """装过则返回版本元组"""
        self.assertTrue(
            siteinstall.is_profile_version_installed((u"1000", )))

    def test_version_string_is_installed(self):
        """字符串版本号也算装了"""
        self.assertTrue(siteinstall.is_profile_version_installed("1000"))
...
class TestQuerySiteInstalled(unittest.TestCase):
    ...
    def test_falls_back_to_portal_setup_when_no_quickinstaller(self):
        ...
        setup_tool = FakeSetupTool(
            {"maitux.esignature:default": (u"1000", )})
    ...
    def test_cache_key_is_per_site(self):
        ...
        installed_site = FakeSite(
            ...
            tools={"portal_quickinstaller": FakeQuickInstaller(
                {"maitux.esignature": True})})
        installed_site.REQUEST = request
        other_site = FakeSite(
            path=("", "MaiLIMS"),
            tools={"portal_quickinstaller": FakeQuickInstaller(
                {"maitux.esignature": False})})
