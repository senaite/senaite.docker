...
**新增字段依赖（未落地前模板相应字段为空/占位）**：储存条件、含量/纯度、放置条件、放置位置见 `../标签设计-缺失字段新增需求说明.md`。

## 安装注册（buildout）

```ini
[buildout]
develop += /opt/addons/customers/maitux.labeldesign
eggs    += maitux.labeldesign
[instance]
zcml    += maitux.labeldesign
[plonesite]
profiles += maitux.labeldesign:default
```
...
目标环境顺序：**先 `maitux.stock:default`、`INNOCARE.arextension:default`，再 `maitux.labeldesign:default`**。对其它 maitux addon 无耦合。

## 卸载

- 执行 `maitux.labeldesign:uninstall` profile 后，移除 `custom-addon.cfg` 三处注册。